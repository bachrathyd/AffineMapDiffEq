...
g = 100000000.17#0.25#gap
Acc = -1.1#0.2#-0.77#0.2          # nat. freq
#Acc = 0.1#0.2          # nat. freq
τ = sqrt(2) / 03.5#2pi/10
Amp = 0.0#1
#T = 2pi * 0.95
Ω = 1.2#1.30065#  0.96#2pi / T
T = 2pi /Ω
...
aaa=plot(sol,dense=false, legend = false)#,xlim=(5,Tlongsim))
...
# # bbb=plot(sol(tend)[1,:],sol(tend)[2,:], legend = false)#,xlim=(-0.6,0.5)
# # xlabel!("x")
# # ylabel!("dx/dt")
...
#savefig( "impact_Acc_feedback_switch_on_long_delay.png")  # or .pdf, .svg, etc.

savefig( "impact_Acc_feedback_switch_on_small_delay_UnStab.png")  # or .pdf, .svg, etc.
