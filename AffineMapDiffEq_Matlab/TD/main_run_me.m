...
for n = 40:-5:10%10:1:100 % Number of discretization elements in the contact patch
dx = 2*a/n;
x = linspace(-a,a,n+1);
% timestep size
dt = dx/(RE*Omega)/2;
...
% par.options = odeset('RelTol',1e-9,'AbsTol',1e-9); %options of the dde solver
% par.options = ddeset('RelTol',1e-6,'AbsTol',1e-6);%,'MaxStep',par.T/100); %options of the dde solver
par.options = ddeset('RelTol',1e-3,'AbsTol',1e-3)%,'MaxStep',par.T/100); %options of the dde solver
...
par.StateSmapled=linspace(-taumax,0,1000);%no-need for uniform sampling
...
% s0start=ones(par.Ndim,par.Nstep)*0.001;
% s0start=rand(par.Ndim,par.Nstep);
s0start=repmat(y0,1,par.Nstep);
tic
...
% opts.v0=zeros(par.Ndim*par.Nstep,1)
% opts.v0(2)=1
opts.maxit=30;
opts.v0=s0flat;
opts.fail='keep';
...
plot(lam_sim,'s','LineWidth',2,'MarkerSize',7), hold on,grid on
% for k=-1:1
...
subplot(2,2,3)
plot(n+ 0*real(lam_sim),real(lam_sim)*n^2,'s','LineWidth',2,'MarkerSize',7),hold on, grid on
...
subplot(2,2,4)
plot(n+ 0*real(lam_sim),real(lam_sim),'s','LineWidth',2,'MarkerSize',7),hold on, grid on
% plot(real(lam_sim)*n^2,'o','LineWidth',2,'MarkerSize',7),hold on, grid on
title('real(lam)')
xlabel('n')
ylabel('real(lam)')
...
subplot(2,2,1)
 plot(real(lam_sim),'o','LineWidth',2,'MarkerSize',7),hold on, grid on
 title('real(lam_i)')
xlabel('i')
ylabel('real(lam)_i')
drawnow
