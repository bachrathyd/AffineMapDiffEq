...
dpdp = dynamic_problemSampled(probMathieu, MethodOfSteps(BS3()), τmax, T;
 Historyresolution=Nstep, eigN=20, zerofixpont=true);
# dpdp = dynamic_problemSampled(probMathieu, MethodOfSteps(BS3()), τmax, T;
  Historyresolution=Nstep, eigN=2, zerofixpont=true);
...
function LinMap(his::funcomp{Float64,SVector{2,Float64}})::funcomp{Float64,SVector{2,Float64}}# where Tout  #{T,Tout}
    #println(his)
    T = 10.0
    ...
    function f(t::Float64)::SVector{2,Float64}
        getvalues(sol, (t + 10.0))
    end
...
function foo(n)
    for i = 1:n
        schursolve(LinMap, his2, 1, :LM,
            KrylovKit.Arnoldi(krylovdim=5, tol=1e-4, verbosity=0))
    end
end
#@profview foo(10)
...
@benchmark getindex(schursolve(LinMap, his2, 2, :LM,
        KrylovKit.Arnoldi(krylovdim=5, tol=1e-5, verbosity=0)), [3, 2, 1, 4])

@time musFun = getindex(schursolve(LinMap, his2, 10, :LM,
        KrylovKit.Arnoldi(krylovdim=15, tol=1e-5, verbosity=3)), [3, 2, 1, 4]);
...
NN = 1#1:2:7)
Δt=median(diff(dpdp.StateSmaplingTime))
scalAmp=0.357#Δt*sqrt(2)*2;#TODO: Why? SA[2] ? 0.357# amplitudo plottolasahoz
plot(dpdp.StateSmaplingTime, getindex.(muaff[2][NN], 1))
plot!(dpdp.StateSmaplingTime, getindex.(muaff[2][NN], 2))
plot!(t, getindex.([musFun[2][NN](tt)*scalAmp for tt in t], 1), width=2, linestyle=:dash)
plot!(t, getindex.([musFun[2][NN](tt)*scalAmp for tt in t], 2), width=2, linestyle=:dash,
    title=abs.([muaff[1][NN]; musFun[1][NN]]))
NN += 1
plot!(dpdp.StateSmaplingTime, getindex.(muaff[2][NN], 1))
plot!(dpdp.StateSmaplingTime, getindex.(muaff[2][NN], 2))
plot!(t, getindex.([musFun[2][NN](tt)*scalAmp for tt in t], 1), width=2, linestyle=:dash)
